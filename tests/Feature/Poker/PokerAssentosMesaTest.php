    ...
    public function test_tela_da_mesa_envia_fluxo_de_assentos_para_o_frontend(): void
    {
        ...
        $this->actingAs($user)
            ...
            ->assertInertia(fn (Assert $page) => $page
                ...
                ->where('table.isLocalMode', false)
                ->where('table.modeLabel', 'Mesa do lobby')
                ->where('table.currentUserId', $user->id)
                ->has('table.reviewChecklist', 6)
                ->where('table.reviewChecklist.0.label', 'Assentos e presença dos jogadores')
                ->where('table.reviewChecklist.0.status', 'ok')
                ->where('table.reviewChecklist.4.label', 'Showdown e hierarquia de mãos blindados')
